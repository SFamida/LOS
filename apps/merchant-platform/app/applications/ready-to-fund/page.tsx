...
export default function ReadyToFundPage() {
  ...
  const fetchApplications = async () => {
    try {
      ...
      const readyApps = response.data.filter((app: Application) => app.status === 'approved');
  ...
  const content = (
    <>
      <div className="mb-4">
        <h2>Ready To Fund</h2>
        <p className="text-muted">Applications approved and ready for funding</p>
      </div>

      <Card>
        <Card.Body className="p-0">
          {applications.length === 0 ? (
            <p className="p-3 text-muted mb-0">No applications ready to fund</p>
          ) : (
